...
To do this, open three CMD windows (Windows) or three Terminal windows (MacOS, Linux) and arrange so you can view them conveniently.
 ...
 1. In the same window run: `java -jar target/microservice-BillingSystem-1.1.0.RELEASE.jar registration` and wait for it to start up
 1. Switch to the second window and run: `java -jar target/microservice-BillingSystem-1.1.0.RELEASE.jar accounts` and again wait for
 it to start up
 1. In the third window run: `java -jar target/microservice-BillingSystem-1.1.0.RELEASE.jar web`
...
in the second and third windows.
 ...
 1. In a new window, run up a second account-server using HTTP port 2223:
     * `java -jar target/microservice-BillingSystem-0.0.1-SNAPSHOT.jar accounts 2223`
